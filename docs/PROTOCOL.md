...
Phase 4 implements authenticated GET list/get. POST create and DELETE remain reserved until their batches. `{runtimeId}` is `hermes` for the single supported installation. Responses omit native filesystem paths and `nativeId`.
