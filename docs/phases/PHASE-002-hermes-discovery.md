...
> Implementation status: COMPLETE — feature work frozen after independent R1 re-audit
> Verification: PASS locally — local Go race is environment-blocked by the absence of a C compiler; exact-commit CI remains mandatory before merge/freeze
...
- distinguish trusted official installation evidence from a safely invocable CLI candidate;
...
- provide a compact Dashboard / Runtimes / Settings Desktop shell;
- provide persistent English and Simplified Chinese UI locales and locale-aware local time display;
...
2. Windows current managed launcher: `%LOCALAPPDATA%\hermes\hermes-agent\bin\hermes.exe`;
3. Windows legacy managed launcher: `%LOCALAPPDATA%\hermes\hermes-agent\venv\Scripts\hermes.exe`;
...
Desktop uses a fixed left sidebar and a bounded right content area:

- Dashboard: local Node status/details plus a concise Hermes summary;
- Runtimes: the complete Hermes discovery surface;
- Settings: only the English / Simplified Chinese language preference.

The language preference takes effect immediately, is persisted locally, and defaults to the supported system locale when no preference exists. All user-visible product copy comes from one typed translation resource. Runtime contract states remain stable keys and are translated only for presentation.

Use TanStack Query for one discovery status surface on the Runtimes page:
...
API timestamps remain RFC 3339 UTC. Desktop formats detection time with the active UI locale and the system time zone; formatter tests inject an explicit locale and time zone so CI does not depend on runner defaults.
...
| Dashboard / Runtimes / Settings navigation | bounded Desktop shell and keyboard-accessible navigation | React |
| English / Simplified Chinese | complete typed copy, immediate persistent switching | React |
| UTC detection timestamp | locale-aware display using an explicitly tested time zone | React unit |
| real Windows installation smoke | an existing official installation never reports `NOT_INSTALLED` | gated Windows smoke |
...
- [ ] relevant local checks and exact-commit CI pass for the amended scope; local checks PASS, exact-commit CI pending;
