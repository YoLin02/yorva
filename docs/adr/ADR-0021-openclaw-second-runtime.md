...
- Status: Accepted for Phase 9 implementation by the Owner on 2026-09-10; qualified and frozen under the Phase 9 baseline
- Date: 2026-09-08
...
5. Use native Windows x64 foreground `gateway run` with canonical named profiles and no native service installation. B0 proved two simultaneous authenticated Gateways at medium integrity and official `gateway stop --force --json` with stopped readback. The release's unmanaged `gateway restart --json` rejected its own listener/lock identity, so YORVA implements Restart as official Stop, confirmed STOPPED, then the same Start path. Final G1 verifies composed restart, isolated deletion and Runtime survival across daemon reconnect. During startup a Windows Job owns the entire process tree; failure/cancellation explicitly terminates and joins its owned processes before closing the Job. Command deadlines remain active until direct-process exit even after both output streams close. Only after the exact profile's authenticated status succeeds does YORVA remove kill-on-close and release the process handle, transferring lifetime to OpenClaw without a retained goroutine or persistent PID authority. Closing Desktop/Node therefore leaves a successfully started Runtime running. No login task, Startup entry, native service, or silent elevated helper is installed.
...
P9 does not claim feature parity. Release pinning and structured-output fixtures need upkeep as upstream evolves. B0 and final G1 established the foreground ownership, authenticated handoff, composed restart, isolated deletion and no-login-entry behavior on OpenClaw 2026.9.3 / Windows x64. See the [final validation](../phases/evidence/PHASE-009-VALIDATION.md) and [audit](../phases/audits/AUDIT-009-openclaw-second-runtime.md) for the qualified candidate and preserved corrections.
