...
$diagnosticLog = Join-Path $env:APPDATA "com.yorva.desktop\logs\install.ndjson"
...
try {
    ...
    if ((Get-ExactProcesses $desktop).Count -ne 1) {
        throw "Daemon recovery changed Desktop process ownership."
    }
    ...
    if ((Get-ExactProcesses $desktop).Count -ne 1 -or (Get-ExactProcesses $daemon).Count -ne 1) {
        throw "Single-instance recovery produced duplicate Desktop or daemon processes."
    }
    ...
    if ((Get-ExactProcesses $desktop).Count -ne 1) {
        throw "Desktop reopen did not recover single ownership."
    }
